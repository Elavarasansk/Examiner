...
import { getAllMyAssignedTest, getAssignedTest, getMyQuestions, submitTestAnswers } from '../reduxFlow/actions';
...
class TestAttendMultiContainer extends Component {
  constructor(props){
    super(props);
    this.state = {
        current:'behaviour',
  ...
  fetchQuestionsAssignedToMe = async (e) => {
    const { id } = this.props.assignedTest.toJS();
  ...
  getQusNumComp = () => {
    const { questionsCount } = this.props.assignedTest.toJS();
    const qusArray=[];
  ...
  getQbDetails = () => {
    const { questionBank } = this.props.assignedTest.toJS();
  ...
  getCountdownTimer = () => {
	    const { allowedTime,testStartTime } = this.props.assignedTest.toJS();
	    let expTimeStamp  = new Date();
	    if(testStartTime){
	    	expTimeStamp = new Date(testStartTime);
	    }
	    expTimeStamp.setSeconds( expTimeStamp.getSeconds() + allowedTime );
  ...
  handleTestSubmit = () => {
    const { dispatch , assignedTest, close } = this.props;
    ...
    confirmUpdate = confirm({
      ...
      async onOk(){
        ...
        await submitTestAnswers(assignedTest.get('id'), dispatch);
        await getAllMyAssignedTest(dispatch, {});
...
function mapStateToProps(state) {
  return {
//    assignedTest: state.get('candidate').get('myTests'),
    questionsAssigned: state.get('candidate').get('myQuestions'),
  };
}

export default withRouter(connect(mapStateToProps)(TestAttendMultiContainer));
